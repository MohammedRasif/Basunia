...
import { ArticleItem } from "@/app/components/landing/ArticleSection";

interface CategoryRelevantArticlesSectionProps {
  title?: string;
  badgeWord?: string;
  seeAllText?: string;
  seeAllHref?: string;
...
export default function CategoryRelevantArticlesSection({
  title = "Relevant",
  badgeWord = "ARTICEL",
  seeAllText = "See All airtical",
  seeAllHref = "/blog",
  articles = [],
}: CategoryRelevantArticlesSectionProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
        ...
        <div className="flex flex-row items-center justify-between gap-4 mb-8 sm:mb-10 lg:mb-12">
          ...
          <h2 className="font-dm-serif-text text-2xl sm:text-3xl md:text-[32px] lg:text-[36px] text-[#141414] font-normal leading-[1.18] tracking-tight">
            <span>{title} </span>
            <span className="uppercase font-normal">{badgeWord}</span>
          </h2>

          {/* See All Link with Top-Right Arrow */}
          <Link
            href={seeAllHref}
            className="group font-geist inline-flex items-center gap-1.5 text-sm sm:text-base md:text-[16px] font-medium text-[#8E1831] hover:text-[#761328] transition-colors whitespace-nowrap"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-7 xl:gap-8">
          {articles.map((article) => (
            ...
            >
              ...
              {/* Publication Date Row with Clock Icon */}
